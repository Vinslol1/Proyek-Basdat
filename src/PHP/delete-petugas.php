...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    try {
        // Validasi input untuk memastikan id ada
        if (empty($id)) {
            die("id harus diisi!");
        }
        ...
        echo "<script>alert('Data petugas berhasil dihapus!'); window.location.href='data-petugas.php';</script>";
    } 
...
<section id="sidebar" class="fixed top-0 left-0 h-screen w-1/6 bg-biru_sidebar flex flex-col px-4 py-20 z-50">
    ...
    <section class="flex flex-col bg-abu2 w-5/6 ml-[16.67%] min-h-screen top-0">
        ...
        <div class="flex flex-col mx-12 my-4 p-4 rounded-lg shadow-md bg-white ">
            <p class="font-bold text-2xl">Konfirmasi Hapus Petugas</p>
            <form class="space-y-6 text-xl p-6" method="GET" action="delete-petugas.php">
                <div class="flex flex-row gap-8">
                    <label for="id-petugas" class="block text-gray-700 font-bold md:w-1/6 text-right">ID</label>
                    <input type="text" id="id-petugas" name="id" class="w-full border border-gray-300 rounded-md p-2 max-w-8xl">
                </div>
